...
export default async function AccountPage() {
  ...
  const [user, orders, attention] = await Promise.all([
    userId
      ? User.findById(userId).select("createdAt").lean<{ createdAt: Date }>()
      : null,
      ...
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#F3E9DF] via-[#F7EFE8] to-[#FBE7E0] p-6 md:p-8">
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            {since && (
              <p className="text-xs text-foreground/60">Member since {since}</p>
            )}
          </div>
        </div>
      </section>
      ...
      {isAdmin && attention && (
        <section className="rounded-2xl bg-gradient-to-br from-[#0F172A] via-[#312E81] to-[#4F46E5] p-5 md:p-6 text-white">
          <div className="flex items-center gap-2 text-sm font-semibold">
            <ShieldCheck size={18} aria-hidden /> You are signed in as store
            admin
          </div>
          <p className="mt-1 text-sm text-indigo-200">
            Admin accounts manage the store and don&apos;t place orders.
            Everything you need lives in the admin console.
          </p>
      ...
      {!isAdmin && (
        <>
          <section className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <StatTile
              label={isAdmin ? "My orders" : "Orders"}
